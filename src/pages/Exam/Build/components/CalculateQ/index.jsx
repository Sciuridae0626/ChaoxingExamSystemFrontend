...
import { Input, Tooltip, InputNumber, Dropdown, Menu, Cascader, Modal, Form, Select, message, Upload, Image, Button } from 'antd'
import { PlusOutlined, InboxOutlined, UploadOutlined } from '@ant-design/icons'
import { nanoid } from 'nanoid'
import { MyIcon } from '../../../../../assets/iconfont.js'
import CalculateQCss from './index.module.css'
...
export default class CalculateQ extends Component {
  ...
  state = { textArea: '', radioValue: '', analysisArea: '',
    ...
    answer: '', picURL: '', picFileList: [], haveUpload: false,
    fileList: [
      {
        uid: '-1',
        name: 'image.png',
    ...
    onBlankChange = (event) => {
      setTimeout(() => {
        let newAnswer = event.target.defaultValue
    ...
    handleDelete = (id) => {
      if(window.confirm(`确定删除第${this.props.index}小题吗？`)){
        this.props.deleteCalculateQ(id)
      }
    }
    /* 执行保存小题的回调 */
    handleSave = (id) => {
      ...
      this.props.saveCalculateQ(id, title, pic, answer, grade)
    }
    /* 执行编辑小题的回调 */
    handleEdit = (id) => {
      this.props.editCalculateQ(id)
    }
    ...
    render() {
      ...
      return (<div className={this.props.isSave ? CalculateQCss.mainWrapper : CalculateQCss.mainWrapperShadow}>
        {/* 计算题题干行 */}
        <div className={CalculateQCss.lineWrapper} style={{color: '#7B7B7B', justifyContent: 'space-between', alignItems: 'flex-start'}}>
          <div className={CalculateQCss.lineWrapper} style={{alignItems: 'flex-start'}}> {this.props.index}.&emsp;
          {this.props.isSave ? <div style={{color: '#1B1B1B', wordBreak: 'break-all'}} onClick={() => {this.handleEdit(this.props.id)}}>{this.props.title}</div>
            ...
            {/* 右上角按钮组 */}
            <div className={CalculateQCss.lineWrapper} style={{ width: '30%', justifyContent: 'flex-end' }}>
        ...
        {/* 设置行 */}
        <div className={CalculateQCss.lineWrapper} style={{ justifyContent: 'space-between', marginTop: 20, alignItems: 'flex-end' }}>
          <div className={CalculateQCss.columnWrapper} style={{ width: '70%' }}>
            {this.props.isSave ? <div style={{marginTop: '-1em'}}>
              ...
              <div className={CalculateQCss.lineWrapper}>
                <nobr style={{marginTop: '0.5em'}}>正确答案：</nobr>
                {this.state.answer === '' ? <nobr style={{marginTop: '0.5em'}} onClick={() => {this.handleEdit(this.props.id)}}>无</nobr>
                  : <nobr style={{marginTop: '0.5em'}} onClick={() => {this.handleEdit(this.props.id)}}>{this.state.answer}</nobr>}
              </div>
            </div>
            : <div>
              ...
              <div className={CalculateQCss.lineWrapper} style={{marginTop: '0.5em'}}>
                <nobr style={{marginTop: '0.5em'}}>正确答案：</nobr>
                <Input onChange={this.onBlankChange} defaultValue={this.props.answer} style={{width: '70%', marginTop: '0.5em'}} />
              </div>
            </div>}
