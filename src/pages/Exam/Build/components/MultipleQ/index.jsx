...
import { Input, Tooltip, Checkbox, InputNumber, Dropdown, Menu, Cascader, Modal, Form, Select, message, Upload, Button, Image } from 'antd'
import { PlusOutlined, InboxOutlined, UploadOutlined } from '@ant-design/icons'
import { nanoid } from 'nanoid'
import { MyIcon } from '../../../../../assets/iconfont.js'
import MultipleQCss from './index.module.css'
...
export default class MultipleQ extends Component {

    state = { textArea: '', checkboxValue: [], analysisArea: '',
    ...
    componentDidMount = () => {
        ...
        this.setState({checkboxValue: this.props.answer, 
    ...
    handleSave = (id) => {
        ...
        let option = this.state.optionList
        let answer = this.state.checkboxValue
        let grade = this.grade.value
        this.props.saveMultipleQ(id, title, pic, option, answer, grade)
    }
    /* 执行编辑小题的回调 */
    handleEdit = (id) => {
        this.props.editMultipleQ(id)
    }
    ...
    render() {
        ...
        return (<div className={this.props.isSave ? MultipleQCss.mainWrapper : MultipleQCss.mainWrapperShadow}>
            {/* 多选题题干行 */}
            <div className={MultipleQCss.lineWrapper} style={{color: '#7B7B7B', justifyContent: 'space-between', alignItems: 'flex-start'}}>
                <div className={MultipleQCss.lineWrapper} style={{alignItems: 'flex-start'}}> {this.props.index}.&emsp;
                ...
                {/* 右上角按钮组 */}
                <div className={MultipleQCss.lineWrapper} style={{ width: '30%', justifyContent: 'flex-end' }}>
            ...
            {/* 设置行 */}
            <div className={MultipleQCss.lineWrapper} style={{ justifyContent: 'space-between', marginTop: 20, alignItems: 'flex-end' }}>
                <div className={MultipleQCss.columnWrapper} style={{ width: '70%' }}>
                    ...
                    <Checkbox.Group className={MultipleQCss.columnWrapper} value={this.props.answer}>
                    {this.state.optionList.map((item, index) => {
                        return <Checkbox value={String.fromCharCode(index+65)} style={{marginTop: '0.5em', marginLeft: 0}} onClick={() => {this.handleEdit(this.props.id)}}>
                ...
                : <div> 
                    ...
                    {this.state.optionList.length ? <Checkbox.Group className={MultipleQCss.columnWrapper} onChange={this.onCheckboxChange} defaultValue={this.props.answer}>
                    {this.state.optionList.map((item, index) => {
                        return <Checkbox value={String.fromCharCode(index+65)} style={{marginTop: '1em', marginLeft: 0}}>
                    ...
                    <div className={MultipleQCss.lineWrapper}>
            ...
            <Modal title="自动换题" maskClosable={false} visible={isAutomaticVisible} onOk={() => this.handleOk('automatic')} okText='开始替换'
                ...
                <Form style={{width: '80%'}}>
                    ...
                    <Select mode="multiple" allowClear style={{width: '100%'}} placeholder="不选则默认任意题型">
                        <Option value='single'>多选题</Option>
