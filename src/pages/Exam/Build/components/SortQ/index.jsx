...
import SortQCss from './index.module.css'
...
export default class SortQ extends Component {
    ...
    onBlankChange = (event, id) => {
      setTimeout(() => {
        ...
        let newAnswer = answer.map((item) => {
            if(id === item.id) item.text = event.target.innerHTML
            return item
        })
    ...
    handleSave = (id) => {
        ...
        this.props.saveSortQ(id, title, answer, grade)
    }
    /* 执行编辑小题的回调 */
    handleEdit = (id) => {
        this.props.editSortQ(id)
    }
    ...
    render() {
      ...
      return (<div className={this.props.isSave ? SortQCss.mainWrapper : SortQCss.mainWrapperShadow}>
        {/* 排序题题干行 */}
        <div className={SortQCss.lineWrapper} style={{color: '#7B7B7B', justifyContent: 'space-between', alignItems: 'flex-start'}}>
          <div className={SortQCss.lineWrapper} style={{alignItems: 'flex-start'}}> {this.props.index}.&emsp;
            ...
            {/* 右上角按钮组 */}
            <div className={SortQCss.lineWrapper} style={{ width: '30%', justifyContent: 'flex-end' }}>
        ...
        {/* 设置行 */}
        <div className={SortQCss.lineWrapper} style={{ justifyContent: 'space-between', marginTop: 20, alignItems: 'flex-end' }}>
        <div className={SortQCss.columnWrapper} style={{ width: '70%' }}>
          {this.props.isSave ? <div className={SortQCss.columnWrapper} style={{marginTop: '-0.5em'}}>
            {this.state.answerList.length ?
              <div className={SortQCss.columnWrapper}><nobr>正序项：</nobr>
                {this.state.answerList.map((item, index) => {
                  return <div className={SortQCss.lineWrapper}>
                    <nobr style={{marginTop: '0.5em'}}>第{index+1}项：</nobr>
                    <div style={{marginTop: '0.5em', wordBreak: 'break-all'}} onClick={() => {this.handleEdit(this.props.id)}}>{item.text}</div>
                  </div>})}
              </div>
              : <nobr style={{marginTop: '0.5em'}} onClick={() => {this.handleEdit(this.props.id)}}>正序项：无</nobr>}
            </div>
            : <div style={{width: '100%'}}>
              <div className={SortQCss.lineWrapper}>
                <nobr style={{display: 'block'}}>正序项：</nobr>
                <Button onClick={this.addBlank} size='small' type='primary' ghost style={{marginRight: '1em'}}>增加下一项</Button>
                <Button onClick={this.deleteBlank} size='small' danger ghost>删除空白项</Button>
              </div>
              {this.state.answerList.map((item, index) => {
                  return <div className={SortQCss.lineWrapper} style={{width: '100%'}}>
                    <nobr style={{marginTop: '0.5em'}}>第{index+1}项：</nobr>
                    <TextArea key={`sort_answer${item.id}`} style={{width: '100%', marginTop: '0.5em'}} autoSize
                        onChange={(event)=>this.onBlankChange(event, item.id)} defaultValue={item.text} />
                  </div>
                })}
            </div>}
        ...
        <Modal title="自动换题" maskClosable={false} visible={isAutomaticVisible} onOk={() => this.handleOk('automatic')} okText='开始替换'
          ...
          <Form style={{width: '80%'}}>
            <Form.Item label="可选题型" name="qType" style={{ marginBottom: '5%', width: '100%' }} >
              ...
              <Option value='connect'>排序题</Option>
